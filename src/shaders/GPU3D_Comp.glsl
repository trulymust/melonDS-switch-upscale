...
uint ResolveNative3DBlock(ivec2 nativePosition)
{
    ...
    // Match the accelerated OpenGL capture path, which resolves the high-res
    // 3D target to 256x192 with GL_NEAREST before display capture reads it.
    ivec2 highResPosition = nativePosition * scale + ivec2(scale >> 1);
    return Resolve3DPixel(highResPosition);
}
